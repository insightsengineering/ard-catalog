...
ard_formatting <- ard_summary(ADSL,
...
To apply the same rounding function as {rtables}, use the `fmt_fun` parameter in the ARD building function followed by the `apply_fmt_fun`. You can also apply the `update_ard_fmt_fun`.
...
```{r}
ard_with_iec_round_option <- ard_summary(ADSL,
  by = TRT01A, variables = "BMIBL",
  fmt_fn = everything() ~ list(everything() ~ "xx")
) |>
  apply_fmt_fun() |>
