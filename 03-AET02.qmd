...
adsl <- syn_data$adsl
adae <- syn_data$adae |>
  filter(AEBODSYS == 'cl A.1' | AEBODSYS == "cl D.1")
...
# bind the ARDs together
final_aet02_ard <- bind_rows(tot_pat_ae, tot_ae, tot_pat_ae_aebod, tot_ae_aebod, ae_n_p_aedecod) |> 
  as.data.frame() |> 
  select(-c(variable, context, warning, error))
