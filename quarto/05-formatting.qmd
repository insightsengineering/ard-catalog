...
```{r}
compare_df <- cbind(ard_formatting[,c("group1_level", "stat_name", "stat", "stat_fmt")], 
...
To apply the same rounding function as {rtables}, use the `fmt_fn` parameter in the ARD building function followed by the `apply_fmt_fn`. 

```{r}
reformat <- ard_continuous(ADSL, by = TRT01A, variables = "BMIBL",
                     fmt_fn = everything() ~ list(everything() ~ round))|>
  apply_fmt_fn() |>
  dplyr::filter(stat_name %in% c("N", "mean", "sd", "min", "max", "median"))

compare_df <- cbind(compare_df, reformat[, "stat_fmt"])
...
t <- ard_formatting |>
  update_ard_fmt_fn(stat_names = "max", 
                    fmt_fn = round)
